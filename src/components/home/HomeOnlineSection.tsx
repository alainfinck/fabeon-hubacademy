...
export function HomeOnlineSection({ courses, embedded = false }: Props) {
  const online = courses
    .filter((c) => c.format === 'en-ligne' || c.format === 'hybride')
    .slice(0, embedded ? 12 : 4)

  return (
    <section
      className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 ${embedded ? 'py-10' : 'py-20'}`}
    >
      {!embedded && (
      ...
      <div className={`grid sm:grid-cols-2 ${embedded ? 'lg:grid-cols-3' : 'lg:grid-cols-4'} gap-6`}>
        {online.map((course) => (
      ...
      {embedded && (
        <div className="mt-10 text-center">
          <Link
            to="/cours"
            className="inline-flex items-center gap-2 text-brand-600 dark:text-brand-400 font-medium text-sm hover:underline"
          >
